...
If `black` is not installed on the machine, use the current public source bootstrap path instead of guessing syntax:
...
Signed GitHub Release archives and the npm wrapper are planned public install paths. Until they are published and verified through the release trust workflow, source bootstrap is the official no-installed-CLI path.
...
- When `black` is unavailable, use the source bootstrap commands from `docs/install.md` and run `go run ./cmd/black ...` from `packages/cli`.
...
For learning examples, an agent may model a simple calculator-like record with input number fields and read-only `computed` fields for `+`, `-`, `*`, and `/`. That is not the same as a button-driven browser calculator.

Do not write unsupported entity validations such as `validate rightValue != 0` today. Current entity validation supports field-to-field comparisons and conditional required rules; use field modifiers such as `min` and `max` when a literal numeric constraint is acceptable, or label the missing literal-comparison validation as future compiler work.
