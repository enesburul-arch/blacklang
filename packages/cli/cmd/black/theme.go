...
func ParseTheme(file string, source string) (ThemeDecl, []Diagnostic) {
	...
	for _, statement := range statements {
		...
		default:
			addDiagnostic(statement.Position, "INVALID_THEME_DECLARATION", fmt.Sprintf("Unsupported .blackthm statement %q.", parts[0]), "Use blackthm, version, target, locked, token, profile, or mode.")
		}
	}
